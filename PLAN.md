...
**ハンズオン**
- AWS アカウント作成（既にあれば確認）。ルートユーザーに MFA。
- 作業用 IAM ユーザー（管理者）を作り、以後ルートは使わない。
- **Budgets で月額アラート**（例: 1,000円 / 3,000円で通知）。Cost Explorer 有効化。
- 課金停止のクセ: 「作ったら消す」を徹底。AWS CLI と AWS 認証情報をローカル設定。
- リージョンは原則 `ap-northeast-1`（東京）で統一。

**成果物**: `knowledge/00-env-setup.md`（手順とハマりどころ）
**チェック**: [ ] MFA済 [ ] IAMユーザー運用 [ ] Budgetsアラート [ ] CLI疎通(`aws sts get-caller-identity`)

> 💡 バイブコーディング: 「使ってないリソースを列挙して消す」棚卸しスクリプトを AI に作らせて毎週末走らせる（コスト事故予防）。
...
- S3 バケットを CLI で作成→ポリシーで公開/非公開を切り替え。
...
**到達目標**: **AWS Certified Solutions Architect – Associate (SAA-C03) 合格**。

**ハンズオン/学習**
- 模試を回し、弱点ドメインを Phase 1-4 のハンズオンで埋める。
- 設計の定石（疎結合/弾力性/コスト最適化/Well-Architected 5本柱）を自分の言葉で説明できるように。
- **受験を予約して受ける**。
...
- GuardDuty / Security Hub / CloudTrail / Config で検知と監査。
- **フォレンジック演習**: 「侵害された想定」のログから何が起きたか追う。

**成果物**: `summary/p8-security.md` + `knowledge/08-incident-response.md`
**チェック**: [ ] クロスアカウントAssume [ ] CloudTrailで操作追跡 [ ] IR手順を1本書く
